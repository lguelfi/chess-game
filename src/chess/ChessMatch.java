...
public class ChessMatch {
    ...
    private Piece makeMove(Position source, Position target) {
        ...
        if (p instanceof King) {
            if (target.getCollumn() == source.getCollumn() + 2) {
                Position sourceR = new Position(source.getRow(), source.getCollumn() + 3);
                Position targetR = new Position(source.getRow(), source.getCollumn() + 1);
                ChessPiece rook = (ChessPiece) board.removePiece(sourceR);
                board.placePiece(rook, targetR);
                rook.increaseMoveCount();
            }
            if (target.getCollumn() == source.getCollumn() - 2) {
                Position sourceR = new Position(source.getRow(), source.getCollumn() - 4);
                Position targetR = new Position(source.getRow(), source.getCollumn() - 1);
                ChessPiece rook = (ChessPiece) board.removePiece(sourceR);
                board.placePiece(rook, targetR);
                rook.increaseMoveCount();
    ...
    private void initialSetup() {
        ...
        placeNewPiece('e', 1, new King(board, Color.WHITE, this));
        placeNewPiece('h', 8, new Queen(board, Color.WHITE));
        placeNewPiece('h', 1, new Queen(board, Color.WHITE));
        placeNewPiece('a', 8, new King(board, Color.BLACK, this));
        placeNewPiece('b', 8, new Pawn(board, Color.BLACK, this));
    }
}
